...
| `~/` | Hardcoded absolute paths under root home |
...
| Pattern | Example | Fix |
|---------|---------|-----|
| Owner's full name | "the operator" | Replace with "the owner" |
| Owner's first name alone | "the operator asks" | Replace with "the owner asks" or "the owner asks" |
...
- [ ] **Gotchas section** — repeats "the operator's token must NEVER..." or account separation reminders
...
- [ ] **Script invocation examples** — `python <hermes-root>/skills/...` (replace with `{skill_root}/...`)
...
| Absolute paths in code examples (`~/`, `/home/`, `/etc/`) | Sensitive File Access | Use `{agent_root}`, `{skill_root}`, or `/path/to/...` placeholder |

**Rule:** If a path appears in a *descriptive or instructional* context (not a Gotcha documenting a legacy path), replace it with functional prose or a template variable. Paths in Gotchas describing problems ("legacy path `~/...` may be stale") are fine — they're documenting a gotcha, not prescribing a path.
