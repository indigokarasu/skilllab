...
_Skills written to `<soul-repo>/skills/` are invisible to the agent unless also installed in the active profile skills directory._

## Problem

The SOUL repo (`<soul-repo>/` or `/indigokarasu/SOUL/`) has a `skills/` directory where new skills are authored. But the agent only scans `~/.hermes/profiles/indigo/skills/` for active skills. A skill that exists in `<soul-repo>/skills/<name>/` will never appear in `skills_list` or `skill_view` until it's copied to the active profile.
...
- Agent searches only `~/.hermes/profiles/indigo/skills/` and misses the `<soul-repo>/skills/` copy
...
Any skill directory in `<soul-repo>/skills/` that is NOT in `~/.hermes/profiles/indigo/skills/` is an install gap.

## Fix

Copy the skill to the active profile:
```bash
cp -r <soul-repo>/skills/<name> ~/.hermes/profiles/indigo/skills/<name>
```
